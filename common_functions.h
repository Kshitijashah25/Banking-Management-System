...
bool loginHandler(bool isAdmin, int connFD, struct Customer *customerPtr) {
    ...
    if(isAdmin) {  // If logging in as Admin
        ...
        strtok(tempBuffer, "-");
        //char *strtok(char *str, const char *delim) The contents of this string are modified and broken into smaller strings (tokens).
...
//atoi function returns the converted integral number as an int value. If no valid conversion could be performed, it returns zero.
        if (accountNumber == 0) {
    ...
    //int ano = offset / sizeof(struct Account);
    //printf("%d" , &ano);
    ...
    {	bzero(writeBuffer, sizeof(writeBuffer));
        writeBytes = write(connFd, "Invalid account number", strlen("Invalid account number"));
    }
    else
    {
